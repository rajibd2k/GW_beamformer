%%% Modified Gaussian Window for Concentric Circular Array (CCA)
%%% Optimize Azimuth-Directivity

%%% h : final beamformer, for all frequencies, for all the concentric rings
%%% beta_prime : (-inf,1] : final gaussian window variance (log100 scale), for all frequencies, for all rings 
...
%%% init_beta_prime : [0,1] : initialization of gaussian window variance (log100 scale), for all frequencies, for all rings 
...
%%% mu_beta_prime : stepsize (= 0.05) for modifying gaussian window variance (log100 scale), for all frequencies 
...
function [ h, beta_prime, w, theta_range, phi_range, Delta_b_theta, Delta_b_phi, D, W] = GradDes_Gaussian_CCA( M_active, init_beta_prime, init_w, num_iterations, gradient_stepsize, mu_beta_prime, mu_w, theta_BW, phi_BW, power_level_diff, r_p, phi_p_m, theta_d, phi_d, f, c, Ts )
...
[ h ] = Modified_Bidir_Gaussian_CCA( M_active, beta, w, r_p, phi_p_m, theta_d, phi_d, f, c, Ts ) ;
% Metrics
...
% --------------------------------------------------------------------------------------------
D_min = 5 ; % 5 dB directivity factor
D_min = 10^(D_min / 10) ;

theta_active = ( Delta_b_theta(1,:) < 0 ).*( D(1,:) >= D_min ) + 0 ;
phi_active = ( Delta_b_phi(1,:) < 0 ).*( D(1,:) >= D_min ) + 0 ;
...
for iteration = 2 : num_iterations
    ...
    for p = 1:P
        ...
        [ tmp_h ] = Modified_Bidir_Gaussian_CCA( M_active, tmp_beta, w, r_p, phi_p_m, theta_d, phi_d, f, c, Ts ) ;
        % Metrics
        F_low = -1000 ; F_high = -1000 ; % Hz (immaterial)
    ...
    for p = 1:P
        ...
        [ tmp_h ] = Modified_Bidir_Gaussian_CCA( M_active, beta, tmp_w, r_p, phi_p_m, theta_d, phi_d, f, c, Ts ) ;
        % Metrics
...
%         w{p} = value ;        
    ...
    theta_active = ( Delta_b_theta(iteration,:) < 0 ).*( D(iteration,:) >= D_min ) + 0 ;
    phi_active = ( Delta_b_phi(iteration,:) < 0 ).*( D(iteration,:) >= D_min ) + 0 ;
    D_active = not( theta_active + phi_active ) + 0 ; 
