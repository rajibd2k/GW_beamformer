...
%%% var_gaussian_p :  gaussian window width-parameter for each ring, for each frequency
%%% ring_weight_p : weight (emphasis) given to each ring (including central sensor), for each frequency

function [ h ] = Modified_Bidir_Gaussian_CCA( M_active, var_gaussian_p, ring_weight_p, r_p, phi_p_m, theta_d, phi_d, f, c, Ts )
...
%         unit_vec_dist = unit_vec_dist / (max_poss_dist -  min_poss_dist) ; % KW
        unit_vec_dist = unit_vec_dist / max_poss_dist ; %% important GW-1, KW-1
...
%         unit_vec_dist = unit_vec_dist / max(unit_vec_dist) ;
    ...
    % gaussian window
    %-------------------------------------------------------------------------------
    Gaussian_window_p = zeros(M_p, length(f)) ;
    var_gaussian = var_gaussian_p{p} ;
    for idx_f = 1 : length(f)
        tmp_window = - m_angle_correlation.^2 / 2 / var_gaussian(idx_f) ;
        tmp_window = exp( tmp_window ) ;
        tmp_window = tmp_window / sqrt( 2*pi*var_gaussian(idx_f) ) ;
        Gaussian_window_p( :, idx_f) = tmp_window ;
    end


    % gaussian window beamformer
    %-------------------------------------------------------------------------------
    h_p = Gaussian_window_p .* ( ones(M_p, 1) * ring_weight_p{p} ) ;
