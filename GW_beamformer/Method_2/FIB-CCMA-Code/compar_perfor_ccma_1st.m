...
j               = sqrt(-1);
theta_s         = 0/180*pi;
f_vect          = [0:100:500 600:200:8000];
f_vect2         = [100:10:8000];
theta           = (-180:2:180)'/180*pi;

%% 1) design the first-order FIB 

r_1             = 0.03;
r_2             = 0.015;
b_vect          = [0.293  0.414  0.293]'; % supercardioid

% 1-1) design the first-order FIB with single ring 3 + 0
P               = 1;
RP_vect         = [r_1];
NP_vect         = [1];
MP_vect         = [3];
...
% 1-2) design the first-order FIB with 5 + 1 
P               = 2;
RP_vect         = [r_1; 0];
NP_vect         = [1; 0];
MP_vect         = [5; 1];
...
% 1-3) design the first-order FIB with 5+3
P               = 2;
RP_vect         = [r_1; r_2];
NP_vect         = [1; 1];
MP_vect         = [5; 3];
...
% 1-4) design the second-order FIB with 8+3+1 
...
text(3820,-9,'(a)','fontsize',FontSize+1,'interpreter', 'latex')
...
text(3820,-47,'(b)','fontsize',FontSize+1,'interpreter', 'latex')
set(gca,'fontsize',FontSize,'fontname','Times new roman');
